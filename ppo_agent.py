...
class PpoAgent(Agent):
    ...
    def __init__(self, **kwargs):
        ...
        self.target_kl = kwargs.get('ppo_target_kl', None)
        ...
        self.avoid_deadly_actions = kwargs.get('ppo_avoid_deadly_actions', True)
        self.avoid_ghost_radius = kwargs.get('ppo_avoid_ghost_radius', 1)
        self.loop_window = max(1, kwargs.get('ppo_loop_window', 12))
        self.revisit_penalty = kwargs.get('ppo_revisit_penalty', 8.0)
        self.empty_step_penalty = kwargs.get('ppo_empty_step_penalty', 2.0)
        self.ghost_danger_penalty = kwargs.get('ppo_ghost_danger_penalty', 25.0)
        ...
        self.last_kl = None
        self.rollout = []
        self.recent_positions = deque(maxlen=self.loop_window)
    ...
    def getAction(self, state):
        ...
        if self.is_eval and not self.eval_stochastic:
    ...
    def _filtered_legal_actions(self, state):
        ...
        if self.avoid_deadly_actions and len(legal_actions) > 1:
            safe_actions = []
            ...
            if safe_actions:
                return safe_actions
        if self.avoid_ghost_radius > 0 and len(legal_actions) > 1:
            ghost_safe_actions = []
            for action in legal_actions:
                try:
                    successor = state.generateSuccessor(self.index, action)
                    ...
                    continue
                min_dist = self._nearest_active_ghost_distance(successor)
                if min_dist is None or min_dist > self.avoid_ghost_radius:
                    ghost_safe_actions.append(action)
            if ghost_safe_actions:
                return ghost_safe_actions
        return legal_actions

    def _nearest_active_ghost_distance(self, state):
        ...
        for ghost in state.getGhostStates():
            if ghost.configuration is None or ghost.scaredTimer > 0:
                continue
            distances.append(manhattanDistance(pac_pos, ghost.getPosition()))
        return min(distances) if distances else None

    def _food_distance(self, state):
        pac_pos = state.getPacmanPosition()
        food_list = state.getFood().asList()
        if pac_pos is None or not food_list:
            return None
        return min(manhattanDistance(pac_pos, food) for food in food_list)

    def _behavior_reward_adjustment(self, state, next_state):
        ...
        if pac_prev is None or pac_next is None:
            return adjustment

        if not self.recent_positions:
            self.recent_positions.append(pac_prev)

        food_eaten = len(state.getFood().asList()) > len(next_state.getFood().asList())
        capsule_eaten = len(state.getCapsules()) > len(next_state.getCapsules())
        productive_step = food_eaten or capsule_eaten

        if not productive_step and pac_next in self.recent_positions:
            adjustment -= self.revisit_penalty

        prev_food_dist = self._food_distance(state)
        next_food_dist = self._food_distance(next_state)
        if (
            not productive_step
            and prev_food_dist is not None
            and next_food_dist is not None
            and next_food_dist >= prev_food_dist
        ):
            adjustment -= self.empty_step_penalty

        min_ghost_dist = self._nearest_active_ghost_distance(next_state)
        if min_ghost_dist is not None:
            if min_ghost_dist <= 1:
                adjustment -= self.ghost_danger_penalty
            elif min_ghost_dist == 2:
                adjustment -= self.ghost_danger_penalty * 0.4

        self.recent_positions.append(pac_next)
        return adjustment
    ...
    def update(self, state, action, nextState, reward, done):
        if self.is_eval or self.last_state is None:
            return None
        reward += self._behavior_reward_adjustment(state, nextState)
    ...
    def _update_from_rollout(self):
        ...
        self.last_kl = total_kl / max(1, update_count)
        self.update_steps += 1
    ...
    def save(self, path):
        if self.network is not None:
            ...
            try:
                ...
                torch.save(self.network.state_dict(), fallback_path)
                self.last_save_path = fallback_path
                print(f"Warning: could not save to {path} ({e}); saved PPO checkpoint to {fallback_path}")
