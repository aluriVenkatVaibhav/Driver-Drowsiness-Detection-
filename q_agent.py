...
class QAgent(Agent):
    ...
    def computeActionFromQValues(self, state, state_key):
        ...
        return self._break_action_tie(state, best_actions)
    ...
    def getAction(self, state):
        ...
        if not self.is_eval and random.random() < self.epsilon:
            return self._exploration_action(state, legal_actions)
    ...
    def _exploration_action(self, state, legal_actions):
        safe_actions = [
            action for action in legal_actions
            if self._action_safety_score(state, action)[0] >= 0
        ]
        candidates = safe_actions if safe_actions else legal_actions
        return self._break_action_tie(state, candidates)

    def _break_action_tie(self, state, actions):
        if not actions:
            return None

        scored_actions = [(self._action_safety_score(state, action), action) for action in actions]
    ...
    def _action_safety_score(self, state, action):
        """
        Heuristic only used for exploration and exact Q-value ties.
        It keeps tabular learning in charge while making unseen states sane.
        """
        try:
        ...
        if successor.isLose():
            return (-1000, -1000, -1000, -1000)
        if successor.isWin():
            return (1000, 1000, 1000, successor.getScore())
        ...
        nearest_active_ghost = min(active_ghost_dists) if active_ghost_dists else 99
        nearest_scared_ghost = min(scared_ghost_dists) if scared_ghost_dists else 99
        food_list = successor.getFood().asList()
        nearest_food = min(manhattanDistance(pac_pos, food) for food in food_list) if food_list else 0

        immediate_danger = -1 if nearest_active_ghost <= 1 else 0
        ghost_margin = min(nearest_active_ghost, 6)
        scared_bonus = 6 - min(nearest_scared_ghost, 6) if scared_ghost_dists else 0
        food_progress = -nearest_food
    ...
    def get_state_key(self, state):
        ...
        danger_close = int(ghost_scared == 0 and ghost_dist <= 1)
