...
def get_kl_value(agent):
    value = getattr(agent, 'last_kl', None)
    if isinstance(value, (int, float)):
        return value
    return None
...
def run_custom_episode(game, pacman, ghosts, rules, is_eval=False, agent_type='dqn', max_steps=1000):
    ...
    while not game.gameOver:
        ...
        if not is_eval and last_states[agentIndex] is not None and hasattr(agent, 'update'):
    ...
    for i, agent in enumerate(game.agents):
        if i == 0 and uses_shaped_pacman_reward(agent_type) and last_states[i] is not None:
            ...
            if i > 0:
                reward = -reward
        terminal_done = game.state.isWin() or game.state.isLose()
        done = terminal_done if agent_type == 'ppo' else True
        
        if not is_eval and last_states[i] is not None and hasattr(agent, 'update'):
            loss = agent.update(last_states[i], last_actions[i], game.state.deepCopy(), reward, done)
            if loss is not None:
                episode_loss += loss
                updates_count += 1
                
        if not is_eval and hasattr(agent, 'final'):
...
def evaluate_training_policy(pacman, ghosts, rules, lay, display, agent_type, episodes, max_steps):
    previous_eval_state = getattr(pacman, 'is_eval', False)
    pacman.is_eval = True
    ...
    try:
        ...
        pacman.is_eval = previous_eval_state
...
def train():
    ...
    checkpoint_eval_episodes = int(config['training'].get('checkpoint_eval_episodes', 0))
    checkpoint_eval_interval = int(config['training'].get('checkpoint_eval_interval', config['training']['save_interval']))
    ...
    for ep in range(num_episodes):
        ...
        if (ep + 1) % 10 == 0:
            ...
            kl_msg = f" | KL: {kl_value:.4f}" if kl_value is not None else ""
            print(
            ...
            checkpoint_win_rate = win_rate
            checkpoint_score = avg_reward
            should_consider_best = checkpoint_eval_episodes <= 0
            if checkpoint_eval_episodes > 0 and checkpoint_eval_interval > 0 and (ep + 1) % checkpoint_eval_interval == 0:
                checkpoint_score, checkpoint_win_rate = evaluate_training_policy(
                    pacman,
                    ghosts,
                    rules,
                    lay,
                ...
                print(
                    f"  Checkpoint eval ({checkpoint_eval_episodes} eps) | "
                    f"Avg Score: {checkpoint_score:.1f} | "
                    f"Win Rate: {checkpoint_win_rate:.2f}"
                )
                should_consider_best = True

            is_better = (
                checkpoint_win_rate > best_win_rate
                or (checkpoint_win_rate == best_win_rate and checkpoint_score > best_avg_reward)
            )
            if should_consider_best and is_better and hasattr(pacman, 'save'):
                best_win_rate = checkpoint_win_rate
                best_avg_reward = checkpoint_score
